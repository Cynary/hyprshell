...
PluginDescriptionInfo init(HANDLE handle) {
    ...
    // clang-format off
    static auto P1 = Event::bus()->m_events.layer.opened.listen(
        [&](PHLLS layer) { onOpenLayerChange(layer, true); }
    );
    static auto P2 = Event::bus()->m_events.layer.closed.listen(
        [&](PHLLS layer) { onOpenLayerChange(layer, false); }
    );
    static auto P3 = Event::bus()->m_events.input.keyboard.key.listen(
        [&](const IKeyboard::SKeyEvent& event, Event::SCallbackInfo& info) { onKeyPress(event, info); }
    );
    static auto P4 = Event::bus()->m_events.input.mouse.button.listen(
        [&](IPointer::SButtonEvent event, Event::SCallbackInfo&) { onMouseButton(event); }
    );
    static auto P5 = Event::bus()->m_events.input.keyboard.focus.listen(
        [&](SP<CWLSurfaceResource> focus) { onKeyboardFocus(focus); }
    );
    // clang-format on
