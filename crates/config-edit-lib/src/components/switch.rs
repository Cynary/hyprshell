use crate::flags_csv;
use crate::util::{handle_key_combo, key_combo_label, SetCursor, SetTextIfDifferent};
...
pub(crate) enum SwitchInput {
    ...
    SetExcludeSpecialWorkspaces(String),
    Delete,
}
...
impl FactoryComponent for Switch {
            ...
            add_row = &gtk::Box {
                ...
                gtk::Box {
                    ...
                    gtk::Entry {
                        ...
                        #[watch]
                        #[block_signal(h_6)]
                        set_text_if_different: &self.config.exclude_special_workspaces,
                        connect_changed[sender] => move |e| {
                            sender.input(SwitchInput::SetExcludeSpecialWorkspaces(e.text().into()));
                        } @h_6,
                    }
                },
            },
    ...
    fn init_model(init: Self::Init, index: &DynamicIndex, sender: FactorySender<Self>) -> Self {
        ...
        key_controller.connect_key_pressed(move |_, val, _, state| {
            match handle_key_combo(val, state) {
                Some(combo) => {
    ...
    fn update(&mut self, msg: Self::Input, sender: FactorySender<Self>) {
        match msg {
            ...
            SwitchInput::SetExcludeSpecialWorkspaces(value) => {
                self.config.exclude_special_workspaces = value;
            }
            SwitchInput::Delete => {
                sender
                    .output_sender()
